#!/usr/bin/env python3
"""
Profile actual mouse interaction using the real TraceManager path.

Usage:
    venv/bin/python pqwave/tests/profile_trace_drag.py tests/rc_100M.raw
"""
...
def main(filepath):
    ...
    # Profile actual mouse drag events
    N_DRAGS = 100
    ...
    total = t1 - t0
    print(f"\n  {N_DRAGS} drag cycles: {total:.3f}s  ({total/N_DRAGS*1000:.1f}ms/drag)")
    ...
    # Check trace item types
    print("\nTrace item types:")
    for var, plot_item, y_axis in tm.traces[:3]:
        print(f"  {var}: {type(plot_item).__name__}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: profile_trace_drag.py <raw_file>")
        sys.exit(1)
    main(sys.argv[1])
