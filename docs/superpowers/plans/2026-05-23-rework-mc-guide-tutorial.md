# Rework Monte Carlo User Guide Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `docs/monte_carlo/guide.md` as a two-part document — compact feature reference tables followed by three worked tutorials with real numbers.

**Architecture:** Single Markdown file rendered by QTextBrowser (existing `_show_mc_guide` callback in `main_window.py`). No code changes. Part 1 provides skimmable reference tables; Part 2 walks through three example files with Input → Steps → Expected Output pattern.

**Tech Stack:** Markdown (rendered by Qt's QTextBrowser)

---

### Task 1: Part 1 — Loading Modes & Display Reference

**Files:**
- Rewrite: `docs/monte_carlo/guide.md` (entire file — this and subsequent tasks progressively build it)
- Reference: `docs/monte_carlo/examples/` (do not modify)

- [ ] **Step 1: Write the guide header and table of contents**

Write the opening of `docs/monte_carlo/guide.md`:
...
- [ ] **Step 2: Write the Loading Modes reference section**
...
- [ ] **Step 3: Write the Display Modes & Control Bar reference section**
...
```bash
git add docs/monte_carlo/guide.md
git commit -m "docs: add Part 1 loading modes and display reference to MC guide"
```

---

### Task 2: Part 1 — Analysis Features & Correlation Reference

**Files:**
- Modify: `docs/monte_carlo/guide.md`

- [ ] **Step 1: Write the Statistical Analysis reference section**
...
- [ ] **Step 2: Write the Correlation Tools reference section**
...
```bash
git add docs/monte_carlo/guide.md
git commit -m "docs: add Part 1 analysis features and correlation reference to MC guide"
```

---

### Task 3: Part 1 — Session API Command Table

**Files:**
- Modify: `docs/monte_carlo/guide.md`

- [ ] **Step 1: Write the Session API command reference table**
...
```bash
git add docs/monte_carlo/guide.md
git commit -m "docs: add Part 1 session API command reference to MC guide"
```

---

### Task 4: Part 2 — Example Overview & RC Filter Loading Tutorial

**Files:**
- Modify: `docs/monte_carlo/guide.md`
- Reference (read-only): `docs/monte_carlo/examples/ngspice/rc_filter_mc.raw`, `docs/monte_carlo/examples/ngspice/rc_filter_params.csv`

- [ ] **Step 1: Write the Part 2 header and example overview**
...
- [ ] **Step 2: Write Example 1 header and Loading section**
...
```bash
git add docs/monte_carlo/guide.md
git commit -m "docs: add Part 2 example overview and RC filter loading tutorial"
```

---

### Task 5: Part 2 — RC Filter Display & Parameter Annotation Tutorial

**Files:**
- Modify: `docs/monte_carlo/guide.md`

- [ ] **Step 1: Write the Display Exploration section**
...
- [ ] **Step 2: Write the Parameter Annotation section**
...
```bash
git add docs/monte_carlo/guide.md
git commit -m "docs: add RC filter display exploration and parameter annotation tutorial"
```

---

### Task 6: Part 2 — RC Filter Sensitivity, Scatter, Yield & Worst-Case Tutorial

**Files:**
- Modify: `docs/monte_carlo/guide.md`

- [ ] **Step 1: Write the Sensitivity and Scatter section**
...
- [ ] **Step 2: Write the Scatter section**
...
- [ ] **Step 3: Write the Yield and Worst-Case section**
...
```bash
git add docs/monte_carlo/guide.md
git commit -m "docs: add RC filter sensitivity, scatter, yield and worst-case tutorial"
```

---

### Task 7: Part 2 — Ring Oscillator Tutorial

**Files:**
- Modify: `docs/monte_carlo/guide.md`
- Reference: `docs/monte_carlo/examples/ngspice/MC_ring.raw`

- [ ] **Step 1: Write the Ring Oscillator tutorial**
...
- [ ] **Step 2: Write the Envelope Display section**
...
- [ ] **Step 3: Write the Statistics and Histogram section**
...
```bash
git add docs/monte_carlo/guide.md
git commit -m "docs: add ring oscillator envelope, stats and histogram tutorial"
```

---

### Task 8: Part 2 — LTspice Tutorial & Standalone Scripting Example

**Files:**
- Modify: `docs/monte_carlo/guide.md`
- Reference: `docs/monte_carlo/examples/ltspice/MonteCarlo.raw`

- [ ] **Step 1: Write the LTspice LC Filter tutorial**
...
- [ ] **Step 2: Write the Standalone Scripting Example**
...
```bash
git add docs/monte_carlo/guide.md
git commit -m "docs: add LTspice stepped loading tutorial and standalone scripting example"
```
...
**Files:**
- Verify: `docs/monte_carlo/guide.md`
- Reference: `docs/monte_carlo/examples/` (read-only)

- [ ] **Step 1: Verify all file paths resolve**

Run:
```bash
python -c "
from pathlib import Path
root = Path('docs/monte_carlo/examples')
files = [
    root / 'ngspice/rc_filter_mc.raw',
    root / 'ngspice/rc_filter_params.csv',
    root / 'ngspice/MC_ring.raw',
    root / 'ltspice/MonteCarlo.raw',
]
for f in files:
...
"
```
Expected: All four paths print "OK".

- [ ] **Step 2: Cross-check quoted numbers against raw data**

Run:
```bash
cd /home/wing/Apps/pqwave.git && venv/bin/python -c "
from pqwave.models.rawfile import RawFile
from pqwave.analysis.multi_run import compute_cross_run_stats, compute_run_measurements, compute_worst_cases, compute_yield, compute_sensitivity
import numpy as np, csv

raw = RawFile('docs/monte_carlo/examples/ngspice/rc_filter_mc.raw')
ds = raw.datasets[0]; mat = ds['data']
nc = {v['name']: i for i, v in enumerate(ds['variables'])}
vout = np.zeros((21, mat.shape[0]))
for i in range(21): vout[i] = np.abs(mat[:, nc[f'v(vout{i})']])

# Verify quoted numbers
stats = compute_cross_run_stats(vout)
assert abs(stats['mean'][0] - 0.3154) < 0.01, f'DC mean mismatch: {stats[\"mean\"][0]}'
assert abs(stats['std'][0] - 0.4586) < 0.01, f'DC std mismatch: {stats[\"std\"][0]}'

with open('docs/monte_carlo/examples/ngspice/rc_filter_params.csv') as f:
    runs = list(csv.DictReader(f))
rvals = np.array([float(r['Rval']) for r in runs])
cvals = np.array([float(r['Cval']) for r in runs])
meas = compute_run_measurements(vout, 'max')
sens = compute_sensitivity(meas, {'Rval': rvals, 'Cval': cvals})
for s in sens:
    if s['param'] == 'Rval':
        assert abs(s['r'] - (-0.042)) < 0.02, f'Rval r mismatch: {s[\"r\"]}'
    if s['param'] == 'Cval':
        assert abs(s['r'] - (-0.208)) < 0.02, f'Cval r mismatch: {s[\"r\"]}'

y = compute_yield(vout, 0.99, 1.01)
assert 10 < np.mean(y) < 20, f'Yield mismatch: {np.mean(y)}'

worst = compute_worst_cases(vout, n=3)
assert worst[0]['run_index'] == 20, f'Worst run mismatch: {worst[0]}'

# Ring oscillator
raw2 = RawFile('docs/monte_carlo/examples/ngspice/MC_ring.raw')
ds2 = raw2.datasets[0]; mat2 = ds2['data']
nc2 = {v['name']: i for i, v in enumerate(ds2['variables'])}
ring = np.zeros((31, mat2.shape[0]))
for i in range(31):
    k = f'v(vout{i})'
    if k in nc2: ring[i] = np.real(mat2[:, nc2[k]])
pkpk = compute_run_measurements(ring, 'pk_pk')
assert 3.09 < pkpk.min() < 3.10, f'pkpk min mismatch: {pkpk.min()}'
assert 3.15 < pkpk.max() < 3.16, f'pkpk max mismatch: {pkpk.max()}'
assert 3.12 < pkpk.mean() < 3.13, f'pkpk mean mismatch: {pkpk.mean()}'

print('All numbers verified OK.')
"
```
Expected: "All numbers verified OK."

- [ ] **Step 3: Verify Markdown renders in QTextBrowser**

Launch pqwave and test:
1. Open the app: `python pqwave.py`
2. **Help > Monte Carlo Guide**
3. Confirm:
   - Table of contents links work
   - All tables render with correct alignment
   - Code blocks render in monospace
   - Sections are clearly separated
4. Skip if running headless — QTextBrowser Markdown support is a Qt built-in.

- [ ] **Step 4: Final commit**

```bash
git add docs/monte_carlo/guide.md
git commit -m "docs: finalize reworked MC user guide with feature reference and tutorials"
```
